...
cs1 = Scale(controlFrame, from_=5, to=30, orient='horizontal', length=200)
...
shortcut_label = ttk.Label(footerFrame,
                           text='Short-Cuts:           Ctrl + r (recalibrate/scale)             Ctrl + i (inverse controls)')
...
def myloop():
    ...
    global blinkCounter, previous_cooldown_time, asking_time, message
    ...
    cv.putText(frame, f"BlinkCounter: [{'infinite' if blinkCounter > 999 else blinkCounter}]", (450, 440),
    ...
    asking_time, message = detector.moveCursor(frame, asking_time, message)

    if blinkLeft and (current_cooldown_time - previous_cooldown_time) > 1 and message == None:
        mouse.click(Button.right, 1)
        blinkCounter += 1
        previous_cooldown_time = time.time()

    if blinkRight and (current_cooldown_time - previous_cooldown_time) > 1 and message == None:
        mouse.click(Button.left, 1)
        blinkCounter += 1
        previous_cooldown_time = time.time()

    if blinkDbl and (current_cooldown_time - previous_cooldown_time) > 1 and message == None:
